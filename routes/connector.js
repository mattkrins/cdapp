...
export default function connector(route) {
    ...
    function hasHandle(haystack = "", needle) { return haystack.includes(`${needle}.`) || haystack.includes(`${needle}/`); }
    function findDependencies(schema, name) {
        for (const rule of schema.rules) {
            if (hasHandle(rule.display, name))
                return rule.name;
            for (const condition of rule.conditions || []) {
                if (hasHandle(condition.key, name))
                    return rule.name;
                if (hasHandle(condition.value, name))
                    return rule.name;
            }
            for (const action of rule.actions || []) {
                if (hasHandle(action.value, name))
                    return rule.name;
                if (hasHandle(action.source, name))
                    return rule.name;
                if (hasHandle(action.target, name))
                    return rule.name;
                if (hasHandle(action.upn, name))
                    return rule.name;
                if (hasHandle(action.ou, name))
                    return rule.name;
                for (const attribute of action.attributes || [])
                    if (hasHandle(attribute.value, name))
                        return rule.name;
                for (const group of action.groups || [])
                    if (hasHandle(group, name))
                        return rule.name;
                for (const template of action.templates || []) {
                    if (hasHandle(template.name, name))
                        return rule.name;
                    if (hasHandle(template.value, name))
                        return rule.name;
                }
            }
        }
    ...
    route.put('/:connector_name', form({
        ...
        try {
            ...
            if (connector_name !== body.name) {
                ...
                if (dependencies)
                    throw reply.code(400).send({ validation: { name: `Found references to name '${connector_name}' in rule '${dependencies}'.` } });
    ...
    route.post('/:connector_name/copy', async (request, reply) => {
        ...
        try {
            ...
            const filePath = `${path}/schemas/${schema.name}/connectors.yaml`;
            writeYAML(schema.connectors, filePath);
            if (copy.id in getHeaders)
                schema.headers[newName] = await getHeaders[copy.id](copy);
            return { connectors: schema.connectors, _connectors: schema._connectors, headers: schema.headers };
        }
        catch (e) {
            const error = _Error(e);
